...
try:
    ...
    INPUT_ZIP_FOLDER = (PLUGIN_DIR / "library_input").resolve()
    OUTPUT_FOLDER = (PLUGIN_DIR / "library_output").resolve()
    for p in (INPUT_ZIP_FOLDER, OUTPUT_FOLDER):
...
def reload_folder_from_path(dpg, folder_path_str: str):
    ...
    """
    if not USE_WX:
        from gui_ui import build_file_list_ui  # DearPyGui version
    else:
        def build_file_list_ui(dpg):  # wx stub
            pass
    ...
    try:
        ...
        log_message(dpg, None, None, f"Scanning folder: {folder_path}")
...
def initial_load(dpg):
    ...
    """
    if not USE_WX:
        from gui_ui import build_file_list_ui, refresh_symbol_list
    else:
        def build_file_list_ui(dpg):  # wx stub
            pass
        def refresh_symbol_list(dpg):  # wx stub
            pass
...
def on_tab_change(dpg, sender=None, app_data=None, user_data=None):
    ...
    if "zip" in active or "import" in active:
        ...
        if not USE_WX:
            ...
            refresh_symbol_list(dpg)
        # else:  wx GUI handles symbol list internally
