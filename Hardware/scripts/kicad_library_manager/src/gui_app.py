...
def toggle_selection_mode(container=None, btn_tag=None):
    """
    Toggles between selecting and deselecting all checkboxes in the given container.
    Works for both ZIP and Project Symbol tabs.
    """
    if not container or not btn_tag:
        log_message(None, None, "[WARN] Invalid toggle_selection_mode call (missing args).")
        return

    if not dpg.does_item_exist(container):
        log_message(None, None, f"[WARN] Container '{container}' not found.")
        return

    # Determine current mode (Select or Deselect)
    btn_label = dpg.get_item_label(btn_tag)
    select_mode = btn_label == "Select All"

    # --- Collect all checkboxes under this container, no matter how they’re nested ---
    def get_all_checkboxes(parent_id):
        checkboxes = []
        if not dpg.does_item_exist(parent_id):
            return checkboxes
        children = dpg.get_item_children(parent_id, 1)
        if not children:
            return checkboxes
        for child in children:
            t = dpg.get_item_type(child)
            if t == "mvAppItemType::mvCheckbox":
                checkboxes.append(child)
            else:
                # recursively dive into groups
                checkboxes.extend(get_all_checkboxes(child))
        return checkboxes

    checkboxes = get_all_checkboxes(container)
    if not checkboxes:
        log_message(None, None, "[WARN] No checkboxes found to toggle.")
        return
    ...
    # Toggle label
    new_label = "Deselect All" if select_mode else "Select All"
    dpg.set_item_label(btn_tag, new_label)
...
def reload_folder_from_path(folder_path_str):
    ...
    try:
        ...
        # Show ZIP action buttons if ZIPs exist
        if valid_paths:
            dpg.show_item("zip_action_group")
        else:
            dpg.hide_item("zip_action_group")

        # Always hide symbol action group on initial load (shown when tab changes)
...
def create_gui():
    ...
    with dpg.window(
        ...
        dpg.add_text(
            "Current Folder: (Initializing...)", tag=CURRENT_PATH_TAG, wrap=0, color=[150, 150, 255]
        )
        dpg.add_separator()
            ...
            with dpg.tab(label="Import ZIP Archives", tag="zip_tab"):
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_button(
                        ...
                        callback=lambda s, a: toggle_selection_mode(container=FILE_CHECKBOXES_CONTAINER, btn_tag="toggle_select_zip_btn"),
                        small=True,
                    )
            ...
            with dpg.tab(label="Export Project Symbols", tag="symbol_tab"):
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_button(
                        ...
                        callback=lambda s, a: toggle_selection_mode(container="symbol_checkboxes_container", btn_tag="toggle_select_symbol_btn"),
                        small=True,
                    )
        ...
        # --- ZIP Action Buttons (Shown only on ZIP tab) ---
        with dpg.group(tag="zip_action_group", horizontal=True, horizontal_spacing=20):
            ...
            # --- Rename Checkbox (Persistent Setting) ---
            rename_chk = dpg.add_checkbox(
                ...
                callback=update_config_rename_checkbox,
            )
        with dpg.tooltip(parent=rename_chk):
            dpg.add_text(
                "If checked, the system renames footprint and 3D model files inside ZIPs "
                "to match the primary symbol name before import."
            )

        # --- Symbol Action Buttons (Shown only on Symbol tab) ---
